-- Fix the streak logic to properly handle missed cleanings
...
BEGIN
  ...
  IF equipment_record.next_cleaning_due IS NOT NULL THEN
    days_since_due := EXTRACT(DAY FROM (cleaned_date - DATE(equipment_record.next_cleaning_due)));
...
-- Also update the function that resets overdue streaks to run properly
...
AS $function$
BEGIN
  -- Reset current_streak to 0 for equipment that is overdue (past the due date)
  UPDATE public.equipment
