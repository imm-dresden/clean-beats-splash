-- First, let's check and update the notification types constraint
...
-- Create trigger for event comment replies (already exists but let's ensure it works)
DROP TRIGGER IF EXISTS event_comment_reply_notification_trigger ON public.event_comments;
